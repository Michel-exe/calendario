...
const promotor = ()=>{
    ...
    const cargarArchivoImagen = (ar) => {
        ...
        reader.addEventListener('load', () => {
            let url = URL.createObjectURL(ar);
            document.querySelector(".inp-img label div").innerHTML=`<img src='${url}' />`
            activarCarga()
        })
    }
    const cargar = (file) => {
    ...
    const subir = async (file)=>{
        ...
        const o = [
            { n: "file", v: file[0]},
            { n: "type", v: "file"},
            { n: "nombre", v: form[1].value},
            { n: "apeP", v: form[2].value},
            { n: "apeM", v: form[3].value},
            { n: "mail", v: form[4].value},
            { n: "phone", v: form[5].value},
            { n: "adress", v: form[6].value},
        ]
        console.log(o);
        peticion(
            rutas.agrProm,
            ...
            (res)=>{
                console.log(res);
                alert(res=="1" ? "Usuario Agregado" : "Error al agregar");
